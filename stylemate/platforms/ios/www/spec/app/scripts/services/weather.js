...
app.factory('WeatherService', ['serverUrl', function (serverUrl) {

    function ForecastIO() {

        this.PROXY_SCRIPT = serverUrl + '/user/getWeather';
    }
    ...
    ForecastIO.prototype.getCurrentConditions = function (latitude, longitude) {
        var data = this.requestData(latitude, longitude);
        if (data !== false) {

            var obj = data.currently;
            obj.localTime = moment(data.currently.time * 1000).zone(data.offset * -60).format('H');

            return new ForecastIOConditions(obj);
        } else {
            return false;
        }
    };
    ...
    ForecastIO.prototype.getForecastToday = function (latitude, longitude) {
        var data = this.requestData(latitude, longitude);
        if (data !== false) {
            var conditions = [];
            var today = moment().format("YYYY-MM-DD");
            for (var i = 0; i < data.hourly.data.length; i++) {
                var raw_data = data.hourly.data[i];
    ...
    ForecastIO.prototype.getForecastWeek = function (latitude, longitude) {
        var data = this.requestData(latitude, longitude);
        if (data !== false) {
            var conditions = [];
            for (var i = 0; i < data.daily.data.length; i++) {
                var raw_data = data.daily.data[i];
                conditions.push(new ForecastIOConditions(raw_data));
            }
            return conditions;
        } else {
    ...
    function ForecastIOConditions(raw_data) {
        ...
        this.getLocalTime = function() {
            ...
            return raw_data.localTime;
        };

        this.getTimeOfDay = function () {

            var localTime = raw_data.localTime;
